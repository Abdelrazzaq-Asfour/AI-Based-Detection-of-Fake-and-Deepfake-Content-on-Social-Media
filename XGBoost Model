...
ber = 1 - recall_score(y_test, y_pred, average='macro')

print("\n Classification Accuracy:")
print(tabulate([["Accuracy XGBoost", f"{accuracy:.2%}", "Overall correctness of the model. Higher is better."]],
...
cutoff = df_grouped['ds'].max() - pd.DateOffset(months=6)
...
if merged_external.empty:
    print("⚠️ 'merged_external' is empty. Please check the merge or date ranges.")
else:

    y_true = merged_external['Fake_News_Count']
    y_pred = merged_external['yhat']
...
if not future_month_df.empty:
    most_common_platform = future_month_df['Platform_Name'].value_counts().idxmax()
    most_common_publisher = future_month_df['Publisher'].value_counts().idxmax()
...
try:
    ...
    used_model = clf
    label_type = "Multiclass (Publishers)"
except:
    y_true = y_test_bin
    y_pred_labels = y_pred_bin
    used_model = model
    label_type = "Binary (Fake / Real)"
